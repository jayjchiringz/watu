...
foreach ($dep in $dependencies) {
    ...
    if (Test-Path $libPath) {
        Write-Host "âœ… Found pre-downloaded $libName in libs â€” skipping extraction."
        continue
    }
    ...
    if (Test-Path $aarPath) {
        Write-Host "ðŸ“¦ Found AAR $aarFilename in libs â€” extracting..."
        ...
        if (Test-Path $classesJar) {
            ...
            Write-Host "âœ… Extracted $libName from AAR."
        } else {
            Write-Error "âŒ classes.jar not found in $aarFilename"
        }
    ...
    Write-Host "ðŸ” Checking Gradle cache: $versionPath"

    if (-not (Test-Path $versionPath)) {
        Write-Error "âŒ Missing Gradle cache: $versionPath"
        continue
    }

    $hashFolder = Get-ChildItem -Directory $versionPath | Select-Object -First 1
    if (-not $hashFolder) {
        Write-Error "âŒ No hash folder in: $versionPath"
        continue
    }
    ...
    if ($aarFile) {
        ...
        if (Test-Path $classesJar) {
            Copy-Item $classesJar $libPath -Force
            Write-Host "âœ… Extracted $($dep.name).jar from AAR"
        } else {
            Write-Error "âŒ No classes.jar in AAR for $($dep.name)"
        }
    } else {
        $jarFile = Get-ChildItem -Path $hashFolder.FullName -Filter "*.jar" | Select-Object -First 1
        if ($jarFile) {
            Copy-Item $jarFile.FullName $libPath -Force
            Write-Host "âœ… Copied $($dep.name).jar directly"
        } else {
            Write-Error "âŒ No AAR or JAR file found for $($dep.name)"
        }
    }
}
