...
$patchSrc = Join-Path $projectRoot "app\src\main\java\com\system\guardian\dex_patch_build"
$buildOut = Join-Path $patchSrc "build\out"
...
$appClasses = Join-Path $projectRoot "app\build\intermediates\javac\debug\compileDebugJavaWithJavac\classes"

$dependencies = @(
    @{ group = "androidx.core"; name = "core"; version = "1.16.0" },
    @{ group = "androidx.work"; name = "work-runtime"; version = "2.10.0" },
    @{ group = "androidx.annotation"; name = "annotation"; version = "1.9.1" },
    @{ group = "com.squareup.okhttp3"; name = "okhttp"; version = "4.10.0" },
    @{ group = "com.squareup.okio"; name = "okio"; version = "3.0.0" },
    @{ group = "com.google.firebase"; name = "firebase-messaging"; version = "24.1.1" }
)
...
foreach ($dep in $dependencies) {
    ...
    if (Test-Path $libPath) {
        Write-Host "✅ Found $libName — skipping"
        continue
    }
    ...
    if ($aarPath -and (Test-Path $aarPath)) {
        Write-Host "📦 Extracting $aarFilename..."
        ...
        if (Test-Path $classesJar) {
            ...
            Write-Host "❌ No classes.jar in $aarFilename"
        }
        continue
    }
    ...
    if (-not (Test-Path $versionPath)) {
        Write-Host "❌ Gradle cache missing: $versionPath"
        continue
    }

    $hashFolder = Get-ChildItem -Directory $versionPath | Select-Object -First 1
    if (-not $hashFolder) {
        Write-Host "❌ No hash folder in: $versionPath"
        continue
    }
    ...
    if ($aarFile) {
        ...
        if (Test-Path $classesJar) {
            ...
            Write-Host "❌ No classes.jar in AAR for $($dep.name)"
        }
    } else {
        ...
        if ($jarFile) {
            ...
            Write-Host "❌ No AAR or JAR found for $($dep.name)"
        }
    }
}

if (-not (Test-Path $patchSrc)) {
    Write-Error "Patch source path not found: $patchSrc"
    exit 1
}
